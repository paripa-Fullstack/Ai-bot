...
# Сообщения для анимации статуса
LOADING_MESSAGES = [
...
async def query_deepinfra_async(prompt, user_id):
    ...
    try:
        ...
        response = await loop.run_in_executor(
            None,
            lambda: requests.post(DEEPINFRA_API_URL, headers=headers, json=data, timeout=60)
        )
...
По вопросам поддержки: @yoursupport
    """
...
⚠️ После проверки платежа администратором ваша подписка будет активирована!
    """
...
async def receipt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Пожалуйста, сохраните ID платежа для связи с поддержкой.",
        parse_mode='Markdown'
    )
...
# Обработка текстовых сообщений (запросы к AI) - УЛУЧШЕННАЯ ВЕРСИЯ
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Отправка начального сообщения о статусе
    status_message = await update.message.reply_text(
        "🤖 *Нейросеть думает...*\n⏳ Анализирую ваш запрос...",
        parse_mode='Markdown'
    )
    
    # Создание события для остановки анимации
    stop_event = asyncio.Event()
    
    # Запуск анимации статуса в фоне
    animation_task = asyncio.create_task(update_status_animation(status_message, stop_event))
    
    try:
        # Запрос к AI (асинхронный)
        ...
        # В случае ошибки остановить анимацию
        stop_event.set()
        try:
            await animation_task
            await status_message.delete()
